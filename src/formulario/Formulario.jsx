...
const Formulario = () => {
  ...
  const onSubmit = async (data) => { // Corrige la firma de esta función para obtener los datos del formulario
    try {
      await inventario({
        CLIENTE: selectEmpresa, // Asegúrate de que estos nombres de propiedad coincidan con los campos requeridos en el servidor
      SERIAL: data.serial,
      ACTIVOFIJO: data.fijo,
      ACTIVOFIJOCARGADOR: data.cargador,
      TIPO: data.tipo,
      DETALLE: data.detalle,
      // Añade los demás campos requeridos aquí
      valorInicial,
    });
  ...
  return (
    <div>
      <button className='buttonSalir' onClick={onLogOut}>Salir</button>
  ...
  readOnly // Esto evita que el usuario modifique manualmente el campo
        ...
        <button type="submit" className="bn5">Enviar</button>
      </form>
    </div>
